...
### 9.使用双向迭代器的写算法，要求双向选代器（`bidirectional iterator`）
...
### 10.划分算法，要求双向选代器（`bidirectional iterator`）
...
### 11.排序算法，要求随机访问迭代器（`random-access iterator`）
...
### 12.使用前向迭代器的重排算法。普通版本在输入序列自身内部重拍元素，_copy 版本完成重拍后写入到指定目的序列中，而不改变输入序列
...
### 13 最小值和最大值，使用 < 运算符或给定的比较操作 comp 进行比较
